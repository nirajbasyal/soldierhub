-- Fix profile sensitive-field trigger after consolidating profile status fields.
-- The profiles table uses verification_status for moderation state. Referencing
-- NEW.status/OLD.status can break admin updates when that field is absent.
