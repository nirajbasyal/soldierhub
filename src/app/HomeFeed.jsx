...
export default function HomeFeed({ initialPosts = [] }) {
  ...
  // Seed from the server-fetched first page so the SSR HTML (and the first
  // client render) contain real post content for crawlers. Using the prop —
  // not localStorage, which is empty on the server — keeps hydration
  // deterministic. The effects below reconcile the live feed + local cache
  // on top once mounted.
  const seededInitialPosts = useMemo(
    () => (initialPosts || []).filter(isValidFeedPost).map(normalizeFeedPostForCard),
    [initialPosts]
  );

  const [cachedPosts, setCachedPosts] = useState(seededInitialPosts);
  const [displayedPosts, setDisplayedPosts] = useState(seededInitialPosts);
  ...
  const hasHydratedLocalCacheRef = useRef(false);

  // After mount only (so it cannot affect hydration), fall back to the local
  // cache when the server seed came back empty — preserves the previous
  // instant/offline first paint for returning users.
  useEffect(() => {
    if (hasHydratedLocalCacheRef.current) return;
    hasHydratedLocalCacheRef.current = true;

    if (seededInitialPosts.length > 0) return;

    const localCached = readCachedFeed();
    if (localCached.length === 0) return;

    setCachedPosts(localCached);
    setDisplayedPosts(localCached);
    displayedPostsRef.current = localCached;
    topFeedMarkerRef.current = buildPostMarker(localCached[0]);
  }, [seededInitialPosts]);
