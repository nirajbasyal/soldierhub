...
export async function POST(request) {
  try {
    ...
    if (userError || !user?.id) {
      return NextResponse.json({ error: "Please log in again before uploading an image." }, { status: 401 });
    }
    ...
    if (!Number.isFinite(size) || size <= 0 || size > maxBytes) {
      ...
      return NextResponse.json({ error: `Compressed image must be under ${maxMb}.` }, { status: 400 });
    }
