...
function getAttemptState(attempt) {
  ...
  if (!hasWaist && !hasHeight) {
    return { complete: false, partial: false, valid: true };
  }

  if (partial) {
    return { complete: false, partial: true, valid: true };
  }
  ...
  return {
    ...
    height,
    rawRatio,
    recordedRatio: truncateToThreeDecimals(rawRatio),
  };
}
...
export default function WHtRCalculator() {
  ...
  const result = useMemo(() => {
    ...
    if (firstAttempt.partial || (firstAttempt.complete && !firstAttempt.valid)) {
      return { error: true };
    }
            ...
            return (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2.5 sm:gap-3">
                  ...
                  <label className="block min-w-0">
                    ...
                    <input
                      ...
                      onFocus={() => nudgeForHeightEntry(index)}
                ...
                {state.complete && state.valid && (
                  <div className="mt-2 text-xs tabular-nums" style={{ color: T.textMuted }}>
                    Attempt WHtR: {state.recordedRatio.toFixed(3)}
                  </div>
                )}
        ...
        >
          ...
          {!result.passes && (
            <p className="text-sm mt-4 leading-relaxed" style={{ color: T.textMuted }}>
              An average recorded WHtR of 0.550 or greater does not meet this calculator&apos;s pass threshold. Official Army screening and any required confirmation measurement must be completed according to current Army policy.
            </p>
          )}
        </section>
      )}
      ...
      <section className="rounded-2xl border p-4 sm:p-5" style={{ backgroundColor: T.surface, borderColor: T.borderSoft }}>
        <div className="flex items-start gap-3">
          ...
          <div className="text-sm leading-relaxed min-w-0" style={{ color: T.textMuted }}>
            <p>
              <strong style={{ color: T.text }}>How this calculator works:</strong> each completed attempt calculates waist circumference ÷ height. When more than one attempt is completed, the calculator averages the completed attempt ratios and then truncates the displayed average to three decimal places.
            </p>
            <p className="mt-2">
              Attempts 2 and 3 are optional. A blank optional attempt is ignored. A partially completed optional attempt is not included in the average.
            </p>
            <p className="mt-2">
              This multi-attempt averaging feature is a convenience calculator and does not replace the official Army Body Composition Program measurement procedure, DA Form 5500, command guidance, or current Army policy.
            </p>
          </div>
        </div>
      </section>
