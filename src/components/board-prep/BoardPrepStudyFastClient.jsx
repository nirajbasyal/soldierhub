...
function getPublication(question) {
  return question?.source_publication || question?.sourcePublication || question?.regulation || question?.publication || question?.source || null;
}
...
function AccuracyNotice() {
  return (
    <div className="-mt-1 flex items-start gap-1.5 rounded-xl border px-2 py-1 text-[10px] font-semibold leading-4 shadow-sm" style={{ backgroundColor: "rgba(255,255,255,0.58)", borderColor: T.borderSoft, color: T.textMuted }}>
      ...
      <p><span className="font-black" style={{ color: T.navy }}>Notice:</span> Soldier Hub questions may be inaccurate or outdated. Verify with official regs/publications and chain of command. Send update/delete requests for fixes.</p>
    </div>
  );
}
...
function RequestSection({ active, tone = "red", icon, title, description, onClick, children }) {
  ...
  return (
    <div className="rounded-[1.4rem] border transition" style={{ backgroundColor: active ? activeBg : T.surface, borderColor: active ? activeBorder : T.borderSoft }}>
      <button type="button" onClick={onClick} className="flex w-full items-start gap-3 p-3 text-left active:scale-[0.995]">
        ...
        <ChevronDown size={18} className={active ? "mt-2 shrink-0 rotate-180 transition" : "mt-2 shrink-0 transition"} style={{ color: T.textSubtle }} />
      </button>
    ...
    try {
      if (mode === "add") {
        ...
        await sendRequest({
          ...
          message: isSingle ? `User submitted one Board Prep flashcard.\n\nQuestion: ${q}\n\nAnswer: ${a}` : `User submitted multiple Board Prep questions for review.\n\n${bulk}`,
        ...
        await sendRequest({
          request_type: action === "delete" ? "remove" : "update",
          question_id: null,
    ...
    <Card className="overflow-hidden">
      ...
      {open && (
        <div className="space-y-3 border-t p-3" style={{ borderColor: T.borderSoft }}>
          <RequestSection active={mode === "add"} tone="red" icon={<PlusCircle size={18} />} title="Add new questions" description="Submit a brand-new board question." onClick={() => toggleMode("add")}>
          ...
          <RequestSection active={mode === "update_delete"} tone="blue" icon={<Search size={18} />} title="Update or remove existing questions" description="Use this when a current question is wrong, outdated, duplicated, or should be removed." onClick={() => toggleMode("update_delete")}>
            <div className="rounded-[1.15rem] border bg-white/70 p-2" style={{ borderColor: T.borderSoft }}>
        ...
        <div className="mt-2 rounded-2xl border p-2" style={{ borderColor: T.borderSoft, backgroundColor: T.surface }}>
          <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
            {filters.map(([value, text]) => (
              <button key={value} type="button" onClick={() => { setStatusFilter(value); setFilterOpen(false); }} className="shrink-0 rounded-full border px-3 py-2 text-xs font-black" style={{ borderColor: statusFilter === value ? T.brandRed : T.border, backgroundColor: statusFilter === value ? T.redBg : T.card, color: statusFilter === value ? T.brandRed : T.textMuted }}>{text}</button>
            ))}
          </div>
        </div>
      )}
      ...
      <button type="button" onClick={() => setOpen((value) => !value)} className="w-full p-4 text-left active:scale-[0.995]">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            {getPublication(question) && <p className="mb-1 text-[11px] font-bold uppercase tracking-[0.16em]" style={{ color: T.textSubtle }}>{getPublication(question)}</p>}
...
export default function BoardPrepStudyFastClient({ initialQuestions = [], initialError = null }) {
  const router = useRouter();
  const hasInitialQuestions = Array.isArray(initialQuestions) && initialQuestions.length > 0;
  const [questions, setQuestions] = useState(() => initialQuestions || []);
  const [loading, setLoading] = useState(() => !hasInitialQuestions && !initialError);
  ...
  useEffect(() => {
    setScores(loadDraftScores((questions || []).map((question) => question.id)));
  }, [questions]);

  const loadQuestionsFallback = useCallback(async () => {
    if (questions.length > 0 || initialError) return;
    setLoading(true);
    setError(null);
  ...
  }, [initialError, questions.length]);

  useEffect(() => { loadQuestionsFallback(); }, [loadQuestionsFallback]);
  ...
  const filteredQuestions = useMemo(() => {
    ...
    return deckQuestions.filter((question) => {
      ...
      return [question.question, question.category, getPublication(question), question.explanation, getAnswerText(question)]
        .filter(Boolean)
        .join(" ")
        .toLowerCase()
        .includes(search);
      ...
      <ToolPage title="Study all questions" eyebrow="Board Prep" icon={BookOpen} onBack={handleTopBack} backLabel={selectedDeck ? "Back to category decks" : "Back to Board Prep"}>
        <div className="space-y-4">
          ...
          {!loading && !error && questions.length > 0 && !selectedDeck && (
            <>
              <ScoreHero summary={allSummary} filteredCount={questions.length} totalCount={questions.length} title="Overall study score" subtitle="all decks" onReset={resetScores} />
              <AccuracyNotice />
              <RequestPanel />
              <DeckBanner />
              <Card className="p-2.5">
              ...
              <div className="grid gap-3 md:grid-cols-2">{filteredDecks.map((deck) => <DeckCard key={deck.key} deck={deck} onSelect={selectDeck} />)}</div>
          ...
          {!loading && !error && questions.length > 0 && selectedDeck && (
            <>
              <ScoreHero summary={selectedSummary} filteredCount={filteredQuestions.length} totalCount={deckQuestions.length} title={`${selectedDeck.label} deck`} subtitle="draft saved" onReset={resetScores} />
